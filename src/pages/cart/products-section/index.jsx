...
const ProductsInCartSection = ( { summaryModal,products } ) => {
    ...
    return (
        <div className={`flex flex-col items-start justify-start h-full w-full py-4 px-6  border rounded-sm shadow-sm mt-4 mb-4 ${summaryModal && size > 400 && 'mb-24'}`}>
                        ...
                        return (
                            ...
                            <div key={product?.id} className='mt-2 flex items-start gap-4 max-[450px]:flex-col max-[450px]:items-center max-[450px]:justify-center'>
                                ...
                                <div className='flex-1  flex flex-col'>
                                    ...
                                    <h2 className='text-sm pt-2 text-neutral-500 leading-3 max-md:text-xs max-[450px]:text-center'>Com desconto no PIX: <span className='font-bold'>{( productPrice - ( productPrice * 0.05 ) )?.toLocaleString( 'pt-BR',{ style: 'currency',currency: 'BRL' } )}</span></h2>
                                    <h2 className='text-sm text-neutral-500 leading-3 max-md:text-xs max-[450px]:text-center'>Parcelado no cartão em até 10x sem juros:<span className='font-bold'>{productPrice.toLocaleString( 'pt-BR',{ style: 'currency',currency: 'BRL' } )}</span></h2>
                                ...
                                <div className='h-full flex-wrap max-md:flex-wrap max-md:flex-col max-md:justify-center  w-[300px] max-[1150px]:w-fit flex items-start justify-between '>
                                    ...
                                    <section className='flex flex-col items-center'>
                                        ...
                                        <p className='font-bold text-orange-500 text-end mt-3 max-sm:mt-1'>{( productPrice - ( productPrice * 0.05 ) )?.toLocaleString( 'pt-BR',{ style: 'currency',currency: 'BRL' } )}</p>
                                    </section>
